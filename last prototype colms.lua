...
elseif tab == 3 then
    ...
    local child_size = imgui.GetWindowSize()
    local draw = imgui.GetWindowDrawList()
    local sepColor = imgui.GetColorU32(imgui.Col.Separator)
    local contentAvail = imgui.GetContentRegionAvail().x
    local colWidth = contentAvail / 2
    local style = imgui.GetStyle()
    local padX = style.WindowPadding.x
    local lineX = child_pos.x + padX + colWidth
    draw:AddLine(imgui.ImVec2(lineX, child_pos.y),imgui.ImVec2(lineX, child_pos.y + child_size.y),sepColor,1)
    imgui.Columns(2, nil, false)
    local headers = { u8"Тест", u8"Тест 1" }
    for i = 1, 2 do
        local text = headers[i]
        local cw, tw = imgui.GetColumnWidth(), imgui.CalcTextSize(text).x
        if cw > tw then imgui.SetCursorPosX(imgui.GetCursorPosX() + (cw - tw) / 2) end
        imgui.Text(text)
        imgui.NextColumn()
    end
    imgui.Separator()
    ...
    for r = 1, numRows do
        for c = 1, 2 do
            ...
            imgui.Dummy(imgui.ImVec2(0, rowHeight))
            local th = imgui.CalcTextSize(text).y
            imgui.SetCursorPosY(startPos.y + (rowHeight - th) / 2)
            imgui.Text(text)
