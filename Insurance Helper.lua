...
local function drawContent()
    ...
    if tab == 1 then
        ...
        imgui.Text(u8"Добро пожаловать в Insurance.help")
        imgui.Dummy(imgui.ImVec2(0, 10))
        imgui.BulletText(u8"Версия: 3")
...
imgui.OnFrame(function() return renderWindow[0] end, function()
    ...
    if imgui.Begin(u8"Insurance.help", renderWindow, imgui.WindowFlags.NoResize) then
        drawSidebar()
        drawContent()
        imgui.End()
    end
        ...
        if skipPlayers[0] then
            for playerId = 0, 999 do
                if sampIsPlayerConnected(playerId) and playerId ~= myId then
                  ...
                  --              sampAddChatMessage(string.format("{00FFFF}[Координаты] {FFFFFF}Игрок %s подошёл к позиции #%d", nick, i), -1)
                   acef.emul("onArizonaDisplay", { text = string.format('window.executeEvent("cef.modals.showModal", `["dialogTip",{"position":"rightBottom","backgroundImage":"bank_notify_add.webp","icon":"icon-info","iconColor":"#FFFF00","highlightColor":"#5FC6FF","text":"Игрок %s подошел к позиции #%d"}]`);', nick, i) })
                                lua_thread.create(function()
                                    wait(3000)
