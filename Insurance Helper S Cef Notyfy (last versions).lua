...
local skipPlayers = imgui.new.bool(false)
...
local playerPoint = {} 
local points = {
    { x = 380.80, y = 852.56, z = 1818.21, radius = 3.0 },
    { x = 1446.81, y = 1922.46, z = 2006.45, radius = 3.0 },
    { x = 1446.81, y = 1924.14, z = 2006.45, radius = 2.5 },
    { x = 1446.82, y = 1925.85, z = 2006.45, radius = 3.5 },
    { x = 1446.81, y = 1927.63, z = 2006.45, radius = 3.0 },
    { x = 1446.82, y = 1929.16, z = 2006.45, radius = 2.8 },
}
...
local function saveSettings()
    ...
    if f then
        ...
        f:write("skipPlayers = " .. tostring(skipPlayers[0]) .. "\n")
                ...
                if line ~= "" and not line:match("^#") then
                    ...
                    elseif mode == "settings" then
                        ...
                        if k and v then
                            ...
                            if k == "skipPlayers" then skipPlayers[0] = v == "true" end
...
local function drawContent()
    ...
    if tab == 1 then
        ...
        imgui.Text(u8("Добро пожаловать в " .. thisScript().name))
        imgui.Dummy(imgui.ImVec2(0, 10))
        imgui.BulletText(u8("Версия: " .. thisScript().version))
       ...
       elseif tab == 2 then
        ...
        if tdClickerActive[0] then
            if imgui.SliderInt(u8"Задержка ТД (мс)", tdClickDelay, 1, 3000) then saveSettings() end
        end        
        if imgui.Checkbox(u8"Уведомление на игроков", skipPlayers) then saveSettings() end        
        if skipPlayers[0] then
            imgui.Indent(20)
            if imgui.Checkbox(u8"Выводить уведомление в чат", notifyChat) then saveSettings() end
            if imgui.Checkbox(u8"Выводить cef уведомление", notifyCef) then saveSettings() end
            imgui.Unindent(20)
        end
...
imgui.OnFrame(function() return renderWindow[0] end, function()
    local windowSize = imgui.ImVec2(600, 440)
    ...
    if imgui.Begin(u8(thisScript().name), renderWindow, imgui.WindowFlags.NoResize) then
        drawSidebar()
        drawContent()
        imgui.End()
    end
...
function main()
    ...
    loadSettings()
    sampRegisterChatCommand("ins", function() renderWindow[0] = not renderWindow[0] end)
    sampAddChatMessage("{00FFFF}[Insurance.help] {FFFFFF}загружен. Команда: {00FFFF}/ins", -1)
    lua_thread.create(function()
        while true do
            wait(500)
            if skipPlayers[0] then
                    ...
                    if sampIsPlayerConnected(playerId) and playerId ~= myId then
                        ...
                        if res and doesCharExist(ped) then
                            ...
                            for i, point in ipairs(points) do
                                ...
                                if dist <= point.radius and not playerPoint[playerId][i] then
                                    playerPoint[playerId][i] = true
                                    local nick = sampGetPlayerNickname(playerId)
                                    if notifyChat[0] then
                                        sampAddChatMessage(string.format("{00FFFF}[Insurance.help] {FFFFFF}Игрок %s подошёл к позиции #%d", nick, i), -1)
                                    end                                    
                                    if notifyCef[0] then
                                        acef.emul("onArizonaDisplay", { text = string.format('window.executeEvent("cef.modals.showModal", `["dialogTip",{"position":"rightBottom","backgroundImage":"bank_notify_add.webp","icon":"icon-info","iconColor":"#2ECC71","highlightColor":"#5FC6FF","text":"Игрок %s подошел к позиции #%d"}]`);', nick, i) })
                                        lua_thread.create(function()
                                            wait(3500)
                                            acef.emul("onArizonaDisplay", { text = 'window.executeEvent("cef.modals.closeModal", `["dialogTip"]`);' })
                                        end)
                                    end                                    
                                elseif dist > point.radius and playerPoint[playerId][i] then
